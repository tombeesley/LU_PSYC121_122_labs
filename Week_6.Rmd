...
We can also remove a column using the select command, by using a "-" in front of any column names. Add a second statement to the code below to remove the "age" column from the data.  
...
Note here that the first argument of the *select* and *filter* functions is the data you are making the operations on. When we %>% the results like this, we no longer need to specify the data argument (it is the result of the previous step).
...
Now you have completed this week's tutorial, it's time to put some of these skills into practice by building an analysis script in R Studio. Go ahead and open up R Studio on your computer. You can find the questionnaire data [at the following link](https://github.com/tombeesley/LU_PSYC121_122_labs/blob/master/data_clean.RData) and you can 


```{r}

filepath <- "https://github.com/tombeesley/LU_PSYC121_122_labs/blob/master/data_clean.RData"
